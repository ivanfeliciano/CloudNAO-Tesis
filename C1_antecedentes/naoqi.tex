...
\noindent\sphinxincludegraphics[scale=0.55]{{nao_config}.png}
...
\sphinxcaption{Lenguajes de programación soportados}\label{\detokenize{chapter_one/naoqi:supp-languages}}
\sphinxaftercaption
...
biblioteca se carga desde \sphinxcode{autoload.ini}, automáticamente al crear una instancia del módulo.
...
Para acceder a los valores almacendos en \sphinxcode{ALMemory}, su usan las funciones:
\begin{itemize}
\item {} 
...
\noindent\sphinxincludegraphics[scale=0.60]{{almotionaxis}.png}
...
El módulo \sphinxcode{ALAnimatedSpeech} brinda la posibildad de hacer que el robot hable
de una manera expresiva.

El funcionamiento de este módulo es como sigue:
\begin{enumerate}
\item {} 
\sphinxcode{ALAnimatedSpeech} recibe texto que puede ser \sphinxstylestrong{anotado} con instrucciones.
...
\item {} 
Cualquier parte del texto que no esté anotado con animaciones se llena con \sphinxstylestrong{modos de lenguaje corporal}.
...
\item {} 
\sphinxcode{ALAnimatedSpeech} hace que el robot diga el texto y se mantenga al tanto del discurspo para lanzar las instrucciones en el momento correcto.

\end{enumerate}

% \subparagraph{Texto anotado}
% \label{\detokenize{chapter_one/naoqi:texto-anotado}}
El texto anotado es una cadena combinando texto que se dirá e \sphinxstylestrong{instrucciones}
...
\item {} 
En el bucle del proceso principal, obtener una imágen llamando a los métodos \sphinxcode{getImageLocal()} o \sphinxcode{getImageRemote()} (dependiedo si el módulo es local o remoto).

\item {} 
Liberar la imágen llamando \sphinxcode{releaseImage()}.
...
\centering
\sphinxcapstartof{table}
\sphinxcaption{Eventos lanzados por los sensores ultrasónicos}\label{\detokenize{chapter_one/naoqi:sonar-events}}
\sphinxaftercaption
\begin{tabulary}{\linewidth}[t]{|T|T|T|}
\hline
\sphinxstylethead{ 
...
es necesario suscribirse al módulo de \sphinxcode{ALSonar} a traveś del método \sphinxcode{subscribe()}.
...
\PYG{n}{motion\PYGZus{}proxy}\PYG{o}{.}\PYG{n}{wakeUp}\PYG{p}{(}\PYG{p}{)} \PYG{c+c1}{\PYGZsh{} Utiliza un método de ALMotion para cambiar la posición del robot}
...
\centering
\sphinxcapstartof{table}
\sphinxcaption{Sitemas operativos compatibles con el SDK de Java}
\sphinxaftercaption
...
Existe un archivo \sphinxcode{.jar} por cada plataforma y versión de NAOqi. Este
archivo \sphinxcode{.jar} permite usar la API de C++ de Aldebaran o crear servicios
...
Una \sphinxcode{Application} es responsable de incializar el marco de trabajo
...
contructor de \sphinxcode{Application}.
...
\PYG{k+kd}{public} \PYG{k+kd}{static} \PYG{k+kt}{void} \PYG{n+nf}{main}\PYG{o}{(}\PYG{n}{String}\PYG{o}{[}\PYG{o}{]} \PYG{n}{args}\PYG{o}{)} \PYG{o}{\PYGZob{}}
  ...
  \PYG{k}{try}\PYG{o}{\PYGZob{}}
      ...
      \PYG{c+c1}{// La sesión puede recuperarse a través del metodo siguiente}
