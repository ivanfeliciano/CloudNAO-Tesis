...
\paragraph{Imagen en vivo de la cámara del robot}
...
\paragraph{Detección de rostros en una fotografía enviada por el robot}
...
\paragraph{Reconocimiento óptico de caracteres y traducción de texto encontrados en una imagen enviada por el robot}
...
Está implicitamente claro que una computadora y acceso a una red inalámbrica
...
\item {} 
\sphinxstylestrong{java}: contiene los archivos de código fuente de Java, incluido el código de prueba JUnit.
...
Una \sphinxstyleemphasis{actividad} es una instancia de \sphinxstylestrong{Activity} una clase del Android SDK.
Una actividad es la encargada de manejar la interacción del usuario con una
pantalla de información.

Se escriben subclases de \sphinxstylestrong{Activity} para implementar la funcionalidad que la
...
subclase de \sphinxstylestrong{Activity} llamada \sphinxstylestrong{QuizActivity}. Quiz Activity manejará la
interfaz de usuario.
...
MainActivity. El archivo de esta clase está en el directorio \sphinxstyleemphasis{app/java} del
...
Por último solo se agrega a las dependencias dentro del archivo \sphinxcode{buildgradle}
...
Para agregar Firebase a tu aplicación, necesitarás un proyecto y un archivo de
...
\item {} 
Para finalizar el proceso, descargarás un archivo \sphinxcode{googleservices.json}.

\item {} 
Si aún no lo haces, copia esto en la carpeta del módulo de tu proyecto, que generalmente es \sphinxstyleemphasis{app/}.
...
Se deben agregar algunas reglas a los archivos \sphinxcode{buildgradle} del proyecto
y de la aplicación. En el primero se incluyen el complemento \sphinxcode{googleservices}
y el repositorio Maven de Google. En el segundo ase agrega la línea
\sphinxcode{applyplugin} en la parte inferior del archivo para habilitar el complemento
de Gradle.

Por cada biblioteca de Firebase que se desee utilizar se añade a las dependencias.
Por ejemplo, sumando a las dependencias de Gradle  \sphinxcode{comgoogle.firebase:firebase-database:11.6.2}
...
El escuchador recine un objeto \sphinxcode{DataSnapshot} que contiene los datos de la
ubicación especifica al momentos del evento.

La diferencia entre \sphinxcode{addValueEventListener()} y
\sphinxcode{addListenerForSingleValueEvent()} es que el primero se suscribe a cierta
...
Para actualizar se llama al método \sphinxcode{updateChildren()} y para elimnar datos
...
\sphinxcode{comfirebase.ui.auth.AuthUI}.
...
puede empezar creando un intent sign-in usando \sphinxcode{AuthUISignInIntentBuilder}.
Se puede recuperar una instancia del contructor llamando
\sphinxcode{createSignInIntentBuilder()} en la instancia retomada de AuthUI.
...
\PYG{n}{startActivityForResult}\PYG{o}{(}
  \PYG{c+c1}{// obteien una instancia de AuthUI basado en la aplicación por defecto}
...
\begin{itemize}
\item {} 
\sphinxcode{ActivityRESULT\_OK}, si esl usuario inició sesión.

\item {} 
\sphinxcode{ActivityRESULT\_CANCELLED}, si el usuario canceló manualmente el inicio de sesión.

\item {} 
\sphinxcode{ErrorCodesNO\_NETWORK}, si no hay conexión a una red.

\item {} 
\sphinxcode{ErrorCodesUNKNOWN\_ERROR},  todos los otros errores.
...
Volley siempre entrea respuestas parseadas sobre el hilo principal. Ejecutarlo
...
\item {} 
\sphinxcode{MenuActivity}, esta actividad es un menú para elegir enntre las actividades restantes. Es un ViewPager, que reacciona al click del usuario.

\item {} 
\sphinxcode{RemoteControllerActivity}, la actividad que corresponde al control remoto. Utiliza módulos de NAOQi, y la base de datos de Firebase para esribir los logs.
