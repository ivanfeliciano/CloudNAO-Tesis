...
\item {} 
\sphinxstylestrong{.gitignore}. Este archivo indica todos los archivos y carpetas que no deben añadirse a un repositorio remoto cuando se usa git.
...
En el archivo \sphinxstyleemphasis{src/App.js} se implementan los
...
\includegraphics[scale=0.4]{react_first_app}
\end{figure}
...
Para ejecutar operaciones de escritura básicas, se usa \sphinxcode{\sphinxupquote{set()}} para guardar
...
se usan los métodos \sphinxcode{\sphinxupquote{on()}} o \sphinxcode{\sphinxupquote{once()}} de \sphinxcode{\sphinxupquote{firebase.database.Reference}}.

El evento más común es \sphinxcode{\sphinxupquote{value}} y permite leer una instantánea estática (snapshot en inglés) del
...
de un recurso, \sphinxcode{resource}, en la base de datos:
...
El escuchador recibe un objeto \sphinxcode{\sphinxupquote{snapshot}} que contiene los datos de la ubicación
...
nodo sin sobrescribir otros nodos secundarios, se usa el método \sphinxcode{\sphinxupquote{update()}}.
...
\PYG{k}{if} \PYG{p}{(}\PYG{n}{user} \PYG{o}{!=} \PYG{n}{null}\PYG{p}{)} \PYG{p}{\PYGZob{}}
  ...
  \PYG{n}{uid} \PYG{o}{=} \PYG{n}{user}\PYG{o}{.}\PYG{n}{uid}\PYG{p}{;}  \PYG{o}{/}\PYG{o}{/} \PYG{n}{El} \PYG{n+nb}{id} \PYG{k}{del} \PYG{n}{usuario} \PYG{n}{es} \PYG{n}{único} \PYG{n}{en} \PYG{n}{un} \PYG{n}{proyecto} \PYG{n}{de} \PYG{n}{Firebase}\PYG{p}{\PYGZcb{}}
\end{sphinxVerbatim}


Para cerrar la sesión de un usuario, se llama a \sphinxcode{\sphinxupquote{signOut}}:
...
Ejecutar \sphinxcode{\sphinxupquote{firebase deploy}} despliega la aplicación en el dominio
\sphinxcode{\sphinxupquote{\textless{}NOMBRE-DE-LA-APP-DE-FIREBASE\textgreater{}.firebaseapp.com}}
