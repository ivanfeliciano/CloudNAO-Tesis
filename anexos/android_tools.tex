...
\item {} 
\sphinxstylestrong{res}: Contiene todos los recursos, como diseños XML, cadenas de IU e imágenes de mapas de bits.
...
La aplicación se llamará Data Structures Quiz. Esta aplicación prueba el
...
Una \sphinxstyleemphasis{actividad} es una instancia de \sphinxcode{Activity}, una clase del SDK de Android.
...
\begin{figure}
    \centering
    \includegraphics[scale=0.20]{view_hierarchy}
...
Cuando un widget está contenido en un \sphinxcode{ViewGroup}, es su hijo. La raíz
...
Al crear el proyecto, se genera una subclase de \sphinxcode{Activity} llamada
\sphinxcode{MainActivity}. El archivo que define a la clase está en el directorio \sphinxstyleemphasis{app/java} del
...
al método \sphinxcode{findViewById}. Este método recibe como parámetro el identificador del recurso
y retorna un objeto View. En nuestro caso se asignan a las variables
\sphinxcode{mAOptionButton} y \sphinxcode{mBOptionButton} los objetos View, a partir del identificadores de los botones.
...
para agregar un \sphinxcode{ValueEventListener} a un objeto \sphinxcode{DatabaseReference}.

El escuchador \sphinxcode{ValueEventListener} de una referencia de la base de datos
tiene una método callback \sphinxcode{onDataChange()} que obtiene una captura estática (snapshot en inglés)
...
Para actualizar se llama al método \sphinxcode{updateChildren()} y para eliminar datos
...
puede empezar creando una intención de inicio de sesión (intent sign-in) usando \sphinxcode{AuthUISign\\InIntentBuilder}.


Se puede recuperar una instancia del constructor llamando
\sphinxcode{createSignInIntent\\Builder()} en la instancia retomada de AuthUI.
...
\fvset{hllines={, ,}}%
\begin{sphinxVerbatim}[commandchars=\\\{\}]
\PYG{c+c1}{// Un valor arbitrario para el código de petición}
...
a la aplicación en el método \sphinxcode{onActivityResult()}.
...
que conlleva un cierre de sesión. El método retorna un objeto \sphinxcode{Task}
...
Para comenzar a utilizar butterknife en un proyecto. Sólo se agrega a las
...
Volley provee una método \sphinxcode{newRequestQueue} que prepara una
\sphinxcode{RequestQueue}, usando valores por defecto, e inicia la cola. En
el siguiente ejemplo se crea una nueva cola de peticiones,
se hace una petición \sphinxcode{GET} a \sphinxcode{www.google.com}, recibe la respuesta
en una cadena y la muestra en un \sphinxcode{TextView}.
